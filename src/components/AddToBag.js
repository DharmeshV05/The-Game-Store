...
export const AddToBag = ({
    ...
    const [, setProductQuantity] = useState(0); // Using state in order for the component to update during the product quantity increase and decrease
    const location = useLocation();
    const [showOrderPlacedNotification, setShowOrderPlacedNotification] =
    useState(false); // New state for the notification
    ...
    function resetCart(){
        ...
        setShowOrderPlacedNotification(true); // Show the "Order Placed" notification
    }
    function hideOrderPlacedNotification() {
        setShowOrderPlacedNotification(false);
      }
    ...
    return (
      ...
      {showOrderPlacedNotification && (
        <motion.div
          className="order-placed-notification"
        ...
        >
          <p>Order Placed!</p>
          <button onClick={hideOrderPlacedNotification}>Close</button>
        </motion.div>
      )}
